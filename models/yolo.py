...
from utils.assigner import dist2bbox, make_anchors
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, v8Detect):
            ...
            self._initialize_v8_biases()  # only run once
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, v8Detect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint):
                    break
    ...
    def _initialize_v8_biases(self):
        """Initialize v8Detect() biases, WARNING: requires stride availability."""
        m = self.model[-1]  # v8Detect() module
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, C2f,
            ...
            args = [c1, c2, *args[1:]]
            if m in [DownC, SPPCSPC, GhostSPPCSPC, C2f,
            ...
            c2 = ch[f] // 2
        elif m in [v8Detect, Detect, IDetect, IAuxDetect, IBin, IKeypoint]:
