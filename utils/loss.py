...
from .assigner import TaskAlignedAssigner, dist2bbox, make_anchors, bbox2dist
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox, tbox[i], xywh=True, CIoU=True).squeeze()  # iou(prediction, target)
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox, selected_tbox, xywh=True, CIoU=True).squeeze()  # iou(prediction, target)
                lbox += (1.0 - iou).mean()  # iou loss
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            pair_wise_iou = box_iou(txyxy, pxyxys)  # pairwise IoU matrix, shape -> (no_targets, no_predictions)
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            a = t[:, 6].long()  # anchor indices
            indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices
            anch.append(anchors[a])  # corresponding anchor dimensions
...
class v8DetectionLoss:
    ...
    def __call__(self, preds, targets):
        ...
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()  # (b, h*w, 4*reg_max)
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()  # (b, h*w, 1)
        ...
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)  # True if row contains gt values, False if row contains only 0
        ...
        return loss[3] * batch_size, loss.detach()  # loss(box, cls, dfl)
